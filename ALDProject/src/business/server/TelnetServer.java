...
public class TelnetServer extends AbstractBasicServer {
	...
	private void addToTree() {
		for (Entry<Integer, City> entry : cityList.entrySet()) {
			vertexTree.add(entry.getValue().getName(),entry.getValue().getId());
		}

		/*
		 * for(City city: cityList.values()) { vertexTree.add(city.getName(),
		 * city.getId()); }
		 */
	}
	...
	public void responseToClient(Socket clientSocket, String message) throws Exception {
		BufferedReader clientResponse = new BufferedReader(new InputStreamReader(clientSocket.getInputStream()));
		...
		if (SearchCriteria.BREITENSUCHE.toString().equals(searchcriteria.toUpperCase())) {
			...
			// arrayGraphBrowser.dijkstraDichteGraphen(arrayGraph, fromId,
			// toId);
